...
def has_right_reversible_stone(board, i, j, color):
    ...
    if j <= SIZE-3 and board[i][j+1] == enemy:
...
def has_lower_reversible_stone(board, i, j, color):
    """ 指定座標の下に返せる石があるか調べる """
    enemy = not(bool(color))
    if i <= SIZE-3 and board[i+1][j] == enemy:
...
def has_right_upper_reversible_stone(board, i, j, color):
    """ 指定座標の右上に返せる石があるか調べる """
    enemy = not(bool(color))
    if i >= 2 and j <= SIZE-3 and board[i-1][j+1] == enemy:
        k = 2
...
def has_left_lower_reversible_stone(board, i, j, color):
    """ 指定座標の左下に返せる石があるか調べる """
    enemy = not(bool(color))
    if j >= 2 and i <= SIZE-3 and board[i+1][j-1] == enemy:
        k = 2
...
def has_right_lower_reversible_stone(board, i, j, color):
    """ 指定座標の右下に返せる石があるか調べる """
    enemy = not(color)
    if i <= SIZE-3 and j <= SIZE-3 and board[i+1][j+1] == enemy:
        k = 2
        while i + k < SIZE-3 and j + k < SIZE-3:
            if board[i+k][j+k] == color:
                return True
            elif board[i+k][j+k] == NONE:
                break
